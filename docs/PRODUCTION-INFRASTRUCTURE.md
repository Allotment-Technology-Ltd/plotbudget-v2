...
**Founding Members (first 100):** Marketing and FAQ state that the first 100 users get 12 months of Premium free. This is enforced in the database: trigger `on_household_created_set_founder` runs after each insert into `public.households`; if fewer than 100 households have founder status, it sets `founding_member_until = NOW() + 12 months` for that household. No manual script required.
