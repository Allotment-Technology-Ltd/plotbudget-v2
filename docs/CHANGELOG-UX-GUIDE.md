...
| File | Purpose |
|------|--------|
| **`CHANGELOG.md`** (repo root) | **Single source of truth.** Generated by semantic-release when branches are merged to `main` (see Release workflow). Prebuild copies it to `apps/marketing/public/changelog.md` so the marketing site always shows the latest releases. |
| **Marketing page** | The “What’s new” page at `/changelog` fetches `changelog.md` and runs it through `sanitizeChangelogContent()` so links, refs, and internal-only lines are stripped for public display. |

**When is it updated?** Every merge to `main` that triggers a release (conventional commits: `feat:`, `fix:`, etc.) runs the Release workflow → semantic-release bumps the version and appends to `CHANGELOG.md` → the next marketing build/deploy uses that file. No manual changelog copy to maintain.
...
## Structure of root `CHANGELOG.md` (and what users see)

Semantic-release writes entries in conventional-changelog format. The sanitizer turns version headers and “Features” / “Bug Fixes” into readable headings and strips refs. The page structure is:
...
1. **Automatically on merge to main:** The Release workflow (`.github/workflows/release.yml`) runs on every push to `main`. It runs migrations, then semantic-release. Semantic-release updates `CHANGELOG.md` from conventional commits and tags the release. The next marketing build (e.g. Vercel deploy of `main`) runs prebuild, which copies `CHANGELOG.md` to `public/changelog.md`, so the changelog page is always in sync with released versions.
2. **Writing good commit messages:** Because the public changelog is derived from root `CHANGELOG.md`, use clear, user-oriented commit messages (e.g. `feat(web): clearer billing copy on pricing page`) so the sanitized output reads well. The UX principles above still apply when you write commits that will end up in the changelog.
